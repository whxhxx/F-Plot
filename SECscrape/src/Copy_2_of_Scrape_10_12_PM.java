...
public class Copy_2_of_Scrape_10_12_PM {
...
//		FileWriter fw = new FileWriter(htmlLink.substring(htmlLink.length()-25, htmlLink.length()-10));
//		fw.write(a);
...
//		getAggAmount2(a);
	...
	private static void getAggAmount2(String a) {

		String textStr[] = a.replaceAll("\\n","///").toUpperCase().split("AGGREGATE AMOUNT BENEFICIALLY"); // \\r/\\n
		int checkPoint = 0;
//		System.out.println(textStr.length);
		ArrayList<String> amountList = new ArrayList<String>();
		for(int j = 1; j < textStr.length; j++){
//			System.out.println(textStr[j]);
					if( textStr[j].matches(".*,?\\d{3}.*")	)
					{
					//	System.out.println("A Num");
						...
						Matcher m4 = Pattern.compile("\\b\\d{1,9}\\b").matcher(textStr[j]);
	...
	private static void getPercent(String a) {
		...
		for (int i = 0; i < textStr.length; i++) 
		{
			...
			{
				for(int j = i; j < i + 16 && j < textStr.length;j++)
				{
...
//			else
//			{
//				if(textStr[i].matches(".*(\\d{1,2})[.](\\d{1,2}).*"))
//				{
//					Matcher m1 = Pattern.compile("\\d{1,2}([.]\\d{1,2})?").matcher(textStr[i]);
//					if (m1.find()) 
//					{
//						String url = m1.group();
